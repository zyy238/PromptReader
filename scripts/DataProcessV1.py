...
f = open('./dep_utils/all_vocab.txt', 'r', encoding='gbk')
dep_vocab = f.read().split()
# print(dep_vocab)
dep_vocab_to_ids = {word: i for i, word in enumerate(dep_vocab)}

data_path = "./data/Original_dataset/v1/"
...
def print_QA(QA: Data.QueryAndAnswer):
    ...
    print(QA.line, '\n',
          ...
          ids_to_tokens(QA.aspect_sentiment_query), '\n', QA.aspect_sentiment_query_mask, '\n',
          QA.aspect_sentiment_query_seg, '\n')
...
def make_QA(line, word_list, aspect_list, opinion_list, sentiment_list, pos_ids_list, dep_list):
    ...
    for i in range(len(aspect_list)):
        ...
        # print('forward_asp_pos=', forward_asp_pos)
        # print('forward_opi_query=', forward_opi_query)
        # print('forward_opi_pos=', forward_opi_pos)
    return Data.QueryAndAnswer(line=line,
                               forward_asp_query=forward_asp_query,
                               forward_opi_query=forward_opi_query,
                               forward_asp_query_mask=forward_asp_query_mask,
...
def line_data_process(line, triplet_list, isQA=True):
    # Line sample:
    # THe perfect spot .####THe=O perfect=O spot=T-POS .=O####THe=O perfect=S spot=O .=O
    # [([1], [3], 2)]
    split = line.split("####")
    # print(split)
    assert len(split) == 3
    ...
    aspect_list = [triplet[0] for triplet in triplet_list]
    aspect_list = [get_start_end(aspect) for aspect in aspect_list]
    if len(aspect_list) > max_aspect_num:
        max_aspect_num = len(aspect_list)
    opinion_list = [triplet[1] for triplet in triplet_list]
    opinion_list = [get_start_end(opinion) for opinion in opinion_list]
    sentiment_list = [triplet[2] for triplet in triplet_list]
...
def train_data_process(text, triplet_data):
    QA_list = []
    max_aspect_num = 0
    max_len = 0
    for i in range(len(text)):
        line = text[i]
        QA, max_aspect_temp, max_len_temp = line_data_process(line, triplet_data[i])
...
def test_data_process(text, triplet_data):
    test_dataset = []
    for index in range(len(text)):
        text_line = text[index]
        word_list, aspect_list_temp, opinion_list_temp, sentiment_list_temp, pos_ids_list, dep_list = line_data_process(text_line,
                                                                                           triplet_data[index],
                                                                                             isQA=False)
        assert len(word_list) == len(pos_ids_list)
        aspect_list = []
        opinion_list = []
        asp_opi_list = []
        asp_sent_list = []
...
def dataset_process():
    for dataset_name in dataset_name_list:
        train_output_path = "./data/preprocess/v1/" + dataset_name + ".pt"
        test_output_path = "./data/preprocess/v1/" + dataset_name + "_test.pt"
        ...
        max_len = 0
        for dataset_type in dataset_type_list:
            text_file = open(data_path + dataset_name + "/" + dataset_type + ".txt", "r", encoding="utf-8")
            triplet_file = open(data_path + dataset_name + "/" + dataset_name + "_pair/" + dataset_type + "_pair.pkl",
                                "rb")
            triplet_data = pickle.load(triplet_file)
            text_lines = text_file.readlines()
            # print('triplet_data=', triplet_data[0])
            # print('text_lines=', text_lines[0])

            # TODO
            QA_list, max_aspect_temp, max_len_temp = train_data_process(text_lines, triplet_data)
            train_dataset_object[dataset_type], max_tokens_temp = tokens_to_ids(QA_list)
            # TODO
            test_dataset_object[dataset_type] = test_data_process(text_lines, triplet_data)
